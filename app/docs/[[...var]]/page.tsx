...
import { ChevronsUpDown, LucideIcon, Rocket } from "lucide-react"
          ...
          <section className="flex flex-col gap-1">
            {
              foundItem.tableOfContents.map((content, idx) => {
                return <>
                  ...
                  {
                    (TypeVal && typeof TypeVal !== "string" && "title" in TypeVal && TypeVal.title === content.title) ? <>
                      <Collapsible
                        open={isOpen}
                      ...
                      >
                        <CollapsibleTrigger asChild >
                          <button className="cursor-pointer flex flex-row gap-2 w-min font-semibold">
                            <a
                              href={`#${content.title}`}
                              onClick={(e) => {
                                e.preventDefault();
                              ...
                              className="light:text-gray-700 light:hover:text-black hover:underline text-base transition-colors duration-200"
                            >
                              {content.title}
                            </a>
                            <ChevronsUpDown />
                        ...
                        <CollapsibleContent className="flex flex-col pl-2 mb-2">
                          {
                            TypeVal.ids.map((id, idx) => {
                              return <a
                                key={idx}
                                href={`#${id}`}
                                onClick={(e) => {
                    ...
                    </> : <a key={idx} href={`/docs${content.path}`} className="light:text-gray-700 light:hover:text-black hover:underline text-base transition-colors duration-200">
                      {content.title}
                    </a>
                  }
                </>
              ...
              <header className="flex w-[78vw] mb-4 flex-row gap-6 justify-between">
                ...
                <section className="w-1/3 flex flex-row gap-2 items-center justify-center">
                  <section className="flex justify-center items-center">
                    <Button className="rounded-xl">Install</Button>
                  </section>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="rounded-xl">
                        <DropdownMenuItem asChild>
                          <OpenInV0Button variant={"secondary"} name={TypeVal.title} className="rounded-xl" />
                        </DropdownMenuItem>
                        <DropdownMenuItem asChild>
                          <Link href={"/"} >
